package server
import("encoding/json";"net/http";"github.com/stockyard-dev/stockyard-quartermaster/internal/store")
...
func(s *Server)routes(){
    s.mux.HandleFunc("GET /health",func(w http.ResponseWriter,r *http.Request){writeJSON(w,200,map[string]string{"status":"ok","service":"stockyard-quartermaster"})})
    s.mux.HandleFunc("GET /api/stats",s.handleOverview)
    s.mux.HandleFunc("GET /api/items",s.handleList)
    s.mux.HandleFunc("POST /api/items",s.handleCreate)
    s.mux.HandleFunc("DELETE /api/items/{id}",s.handleDelete)
