...
func (s *Server) activateLicense(w http.ResponseWriter, r *http.Request) {
	...
	if err := PersistLicense(s.dataDir, key); err != nil {
		log.Printf("quartermaster: license persist failed: %v", err)
	...
	s.limits = ProLimits()
	s.limMu.Unlock()
	log.Printf("quartermaster: license activated via dashboard, persisted to %s/%s", s.dataDir, licenseFilename)
...
func (s *Server) tierInfo(w http.ResponseWriter, r *http.Request) {
	...
	if tier == "trial-required" {
		...
		resp["message"] = "Your trial is not active. Reads work, but you cannot add or change inventory until you start a 14-day trial or activate an existing license key."
	} else {
