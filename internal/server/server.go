...
s.mux.HandleFunc("GET /api/inventory",s.list);s.mux.HandleFunc("POST /api/inventory",s.create);s.mux.HandleFunc("GET /api/inventory/{id}",s.get);s.mux.HandleFunc("DELETE /api/inventory/{id}",s.del)
...
func(s *Server)list(w http.ResponseWriter,r *http.Request){wj(w,200,map[string]any{"inventory":oe(s.db.List())})}
func(s *Server)create(w http.ResponseWriter,r *http.Request){var e store.InventoryItem;json.NewDecoder(r.Body).Decode(&e);if e.Name==""{we(w,400,"name required");return};s.db.Create(&e);wj(w,201,s.db.Get(e.ID))}
...
func(s *Server)stats(w http.ResponseWriter,r *http.Request){wj(w,200,map[string]int{"inventory":s.db.Count()})}
