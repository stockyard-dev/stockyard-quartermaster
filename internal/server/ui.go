...
async function load(){var r=await fetch(A+'/inventory').then(function(r){return r.json()});items=r.inventory||[];document.getElementById('stats').innerHTML='<div class="st"><div class="st-v">'+items.length+'</div><div class="st-l">Total</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div>';render();}
function render(){var q=(document.getElementById('search').value||'').toLowerCase();var f=items;
if(q)f=f.filter(function(i){return (i.name||'').toLowerCase().includes(q)||(i.category||'').toLowerCase().includes(q)||(i.location||'').toLowerCase().includes(q)||(i.quantity||'').toLowerCase().includes(q)});
...
if(i.category)h+='<div class="item-sub">'+esc(i.category)+'</div>';
...
if(i.location)h+='<span>'+esc(i.location)+'</span>';
if(i.quantity)h+='<span>'+esc(i.quantity)+'</span>';
if(i.purchase_date)h+='<span>'+esc(i.purchase_date)+'</span>';
if(i.purchase_price)h+='<span>'+esc(i.purchase_price)+'</span>';
if(i.condition)h+='<span>'+esc(i.condition)+'</span>';
if(i.serial_number)h+='<span>'+esc(i.serial_number)+'</span>';
h+='<span>'+ft(i.created_at)+'</span></div>';

h+='</div>';});document.getElementById('list').innerHTML=h;}
async function del(id){if(!confirm('Delete?'))return;await fetch(A+'/inventory/'+id,{method:'DELETE'});load();}
function formHTML(item){var i=item||{name:"",category:"",location:"",quantity:"",purchase_date:"",purchase_price:"",condition:"",serial_number:"",notes:""};var isEdit=!!item;
var h='<h2>'+(isEdit?'EDIT':'NEW')+' QUARTERMASTER</h2>';
...
h+='<div class="fr"><label>Location</label><input id="f-location" value="'+esc(i.location)+'"></div>';
h+='<div class="fr"><label>Quantity</label><input id="f-quantity" value="'+esc(i.quantity)+'"></div>';
h+='<div class="fr"><label>Purchase Date</label><input id="f-purchase-date" value="'+esc(i.purchase_date)+'"></div>';
h+='<div class="fr"><label>Purchase Price</label><input id="f-purchase-price" value="'+esc(i.purchase_price)+'"></div>';
h+='<div class="fr"><label>Condition</label><input id="f-condition" value="'+esc(i.condition)+'"></div>';
h+='<div class="fr"><label>Serial Number</label><input id="f-serial-number" value="'+esc(i.serial_number)+'"></div>';
...
async function submit(){var body={name:document.getElementById('f-name').value.trim(),category:document.getElementById('f-category').value.trim(),location:document.getElementById('f-location').value.trim(),quantity:document.getElementById('f-quantity').value.trim(),purchase_date:document.getElementById('f-purchase-date').value.trim(),purchase_price:document.getElementById('f-purchase-price').value.trim(),condition:document.getElementById('f-condition').value.trim(),serial_number:document.getElementById('f-serial-number').value.trim(),notes:document.getElementById('f-notes').value.trim()};
if(editId){await fetch(A+'/inventory/'+editId,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}
else{await fetch(A+'/inventory',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}closeModal();load();}
